...
def norm_spec(i, local_dist=100, poly_deg=10, fitcont_mode='poly', plot_rest_frame=True, plot_lya_forest=True, plot=False):
	z_plot_rest_frame, lya_toplot = rest_fram_pars(matched['z'][i],plot_rest_frame)
	if plot_lya_forest: lya_tocut = lya_toplot
	else: lya_tocut = None

	koa_spec = trim_koaspec(i, plot_rest_frame=plot_rest_frame) # make spec connected
...
if __name__ == '__main__':
	...
	lam, flux, _ = norm_spec(i, fitcont_mode=mode, plot=1, plot_rest_frame=False)
